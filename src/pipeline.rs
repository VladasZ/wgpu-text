...
impl Pipeline {
    ...
    /// so an appending write must not touch bytes an earlier recorded draw
    /// still reads. The buffer is bump-allocated: a plain write starts over
    /// at the beginning and an appending write goes after the previous one.
    /// Draws recorded with the old buffer keep it alive when it is
    /// replaced here.
    ...
    /// past them so a later appending write does not overwrite what the
    /// reused draw reads.
